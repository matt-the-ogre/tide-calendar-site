...
- **Egress note for agents**: the Claude-Code-on-the-web sandbox cannot reach `tidecalendar.xyz`/`dev.tidecalendar.xyz`/the NOAA API (egress policy → `403`), and `gh`/direct git pushes are limited to the agent's designated branch. Use the GitHub MCP tools for PRs/merges, and hand `/health`-check and prod-container commands to a human with SSH/laptop access.

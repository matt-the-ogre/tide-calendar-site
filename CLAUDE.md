...
- Automatic deployments via GitHub webhook on pushes to `main` branch (prod) — `development` also auto-deploys to dev.tidecalendar.xyz the same way (confirmed: `/health` commit hash updates ~20-30s after push, no manual trigger)
...
- **Tide Adapters**: `tide_adapters.py` provides unified interface for NOAA and CHS APIs with retry logic; raises `TideServiceUnavailableError` to flag an upstream outage (vs returning `None` for genuine no-data)
...
2. **PDF Generation with Caching**:
   ...
   - If not cached: `get_tides.generate_calendar()` in-process → **raw-data cache check** (`get_tides.get_tide_data()`) → NOAA/CHS API fetch on miss → pcal conversion → ps2pdf → atomic rename into cache (intermediates live in a temp dir; pcal/ps2pdf have 60s timeouts)
   - **Two-layer cache**: the PDF cache is per-unit (`_ft`/`_m`), but the underlying tide data is unit-independent, so `get_tide_data()` caches the standardized CSV per `(station, year, month)` under `PDF_OUTPUT_DIR/rawdata/`. A cache hit there never calls the API, so the second unit renders without a second fetch AND a cache-missed PDF survives an upstream outage once the month has been fetched once (see the upstream-outage note below). Swept at startup alongside PDFs.
   - **API Retry Logic**: 3 attempts with exponential backoff for 502/503/504 gateway errors; a persistent gateway/timeout/network failure raises `TideServiceUnavailableError` (an outage), distinct from a 200-with-no-data result
   - Old-month PDFs and raw-data files are swept once at container startup (`run.py`), not per request
...
5. **Error Handling**: Missing/empty PDFs and invalid inputs trigger custom error templates. Upstream API **outages** (`upstream_unavailable`) are distinguished from a station genuinely having **no predictions** (`no_predictions`) so the user sees "try again in a few hours" instead of "pick a different station" (see the upstream-outage note below)
...
- **`get_tides.py` CLI reads the DB as-is**: unlike the Flask app (which runs `database.init_database()`'s column-migration bootstrap at startup), the standalone CLI doesn't — a stale local `tide_station_ids.db` missing a newer column (e.g. `timezone`) raises `no such column` errors when run directly. Delete the local DB or run via `flask run`/Docker instead to pick up schema changes.
...
- **Raw tide-data cache**: `get_tides.get_tide_data()` caches the unit-independent standardized CSV per `(station, year, month)` under `PDF_OUTPUT_DIR/rawdata/` (`tidedata_<station>_<YYYY>_<MM>.csv`), written atomically and validated on read. It's a second cache layer beneath the per-unit PDF cache: once a month is fetched for either unit, the other renders without a second API call, and a cache-missed request survives an upstream outage if the month was ever fetched. Swept at startup by `cleanup_previous_month_pdfs` (shared `_sweep_previous_month_files`). To pre-warm a hot station so it's never a cold miss, run the CLI for both units (e.g. `python get_tides.py --station_id 9449639 --year 2026 --month 8 --unit imperial`).
- **Upstream outage vs no-data**: adapters raise `tide_adapters.TideServiceUnavailableError` only when an API is unreachable after retries (gateway 5xx / timeout / network); a definitive non-gateway response (e.g. 404) or 200-with-no-data returns `None`. A definitive answer from one endpoint wins over a transient blip on its mirror (CHS has two base URLs; this also covers the CHS numeric-code→UUID `/stations` lookup, the common Canadian path). `calendar_service` maps it to `error_code='upstream_unavailable'`; `routes.py` shows a "service temporarily unavailable, try again in a few hours" message and the quick API returns **HTTP 503** (genuine no-data stays the "no predictions / pick a different station" message + 500). Tests: `test_tide_service_resilience.py`.
- **Debugging "dev works, prod doesn't" for the same station**: it's almost always **cache asymmetry**, not code/DB — dev rebuilds its DB + may hold a cached PDF; prod runs a warm persistent-volume DB and a different PDF cache. The 2026-06 Point Roberts incident was a global **NOAA `datagetter` outage** (HTTP 504 for all stations/months) that only surfaced on prod because prod lacked a cached *imperial* (`_ft`) PDF and had to fetch live, while dev served a cached one and the *metric* (`_m`) cache hit masked it. Get prod ground truth by `docker exec`-ing the prod container: query `/data/tide_station_ids.db` for the station row (`api_source`, `timezone`, coords), query `usage_events` for the real `error_detail` (`no_predictions` vs `upstream_unavailable`), and call the adapter directly (`cd /app && python3 -c "import tide_adapters as t; t.NOAAAdapter().get_predictions('9449639',2026,8)"`). A NOAA outage serves NOAA's own nginx `504` page (front door reachable, backend timing out) — distinct from an IP block (which returns `403`/`429`).
- **Warm-DB self-heal (US stations)**: `database.import_stations_from_csv()` no longer short-circuits on a populated DB — it re-applies the canonical CSV's metadata (`api_source`, `country`, coords, `timezone`, `place_name`) to US rows on every startup so a drifted warm-prod row self-heals (idempotent, ~2100 rows in one transaction, never touches `lookup_count`). Canadian rows already self-heal via the CHS sync upsert. Test: `test_us_station_resync.py`.
- **Egress note for agents**: the Claude-Code-**on-the-web** sandbox specifically cannot reach `tidecalendar.xyz`/`dev.tidecalendar.xyz`/the NOAA API (egress policy → `403`), and `gh`/direct git pushes are limited to the agent's designated branch there — use the GitHub MCP tools for PRs/merges, and hand `/health`-check and prod-container commands to a human with SSH/laptop access. **Local/CLI sessions are not restricted this way** — `curl` to both hosts' `/health` and rendered pages works directly, so verify deploys yourself (e.g. poll `/health` for the new `commit_hash` in a retry loop) instead of handing off.
...
- **Testing pcal/PDF changes**: Cached PDFs mask code changes. Clear cache before testing: `ssh captain "docker exec <container> sh -c 'rm -f /data/calendars/*.pdf'"` (must use `sh -c` for glob expansion in `docker exec`). No SSH access (e.g. an agent session where the 1Password SSH agent can't get an interactive approval)? Request an unusual `year` (current year + 2, say) for the same station/month instead — guaranteed cache miss, no cache-clearing needed; confirm via the response's `Last-Modified` header matching request time. To see the actual rendered layout: `docker build -t tide-calendar-app-test .` → `docker run -d -p 5099:80 --name tide-test tide-calendar-app-test` → `curl -X POST http://localhost:5099/ -d "station_id=9449639&year=2026&month=8&unit=imperial" -o out.pdf` → `magick -density 200 out.pdf out.png` (ImageMagick `magick`/`convert`; `pdftoppm` isn't installed by default on macOS) to view it.
...
- **pcal star-on-date gotcha**: appending `*` to a date field (e.g. `7/15*`) makes pcal both colour-code that day's number (via `-s`) AND render that entry **before** any non-starred entries in the same day's cell, regardless of file order or the event's actual time. `convert_tide_data_to_pcal` relies on the colour effect for low-tide days (`LOW_TIDE_THRESHOLD`) but must avoid the reordering effect, so it stars a separate, text-less `mm/dd*` marker line rather than the tide event line itself — never star a line that has real display text unless you want it pinned first in the cell.
...
- **Analytics**: Server-side `usage_events` table logs every request to `/` and `/api/generate_quick` (station_id, station_name, year, month, status, error_detail, source). No PII. `source='web'` for form submissions, `source='quick_api'` for embed/widget traffic. `status` is `success` | `error` | `rejected` (junk short-circuited before any work — all-zeros IDs via `calendar_service.is_junk_station_id`; logged `junk_station_id`, counts as neither success nor error). Errors are split client (4xx) vs server (5xx) by `database.CLIENT_ERROR_DETAILS`; the dashboard surfaces `client_error_count`/`server_error_count`/`rejected_count` so bot/probe traffic can't read as a server outage. An upstream-API outage logs `error_detail='upstream_unavailable'` (currently buckets as a server error — it's not in `CLIENT_ERROR_DETAILS`); a spike of these means NOAA/CHS is down, not our app. Dashboard at `/admin/analytics` — accepts `Authorization: Bearer $ANALYTICS_TOKEN` header (preferred, stays out of access logs) or `?token=$ANALYTICS_TOKEN` query param (fallback). Returns 404 on any unauth/unconfigured request (invisible to scanners). Events older than 365 days are pruned on container startup. Complements client-side Plausible (which misses cached serves, validation errors, and ad-blocked users). Tests: `python3 scripts/test_usage_events.py`.
- **Unit toggle (metric/imperial)**: Display-only conversion (`app/units.py`); internals (low-tide asterisk threshold, extreme-tide ranking) always use raw metric values. Unit is part of the PDF cache key (`_ft.pdf`/`_m.pdf` suffix in `pdf_filename_for`), so metric and imperial PDFs cache independently. Web form reads `unit` hidden field (default `imperial`). Frontend: ft/m pill group mirrors the country-filter pills; `unit_preference` cookie persists the selection; changing the country filter only updates the toggle when NO cookie is set (cookie wins). Quick API (`/api/generate_quick`): accepts optional `unit` in JSON body; if absent, defaults by station country (USA→imperial, Canada→metric) via `get_station_info`. **Always generates the current month — `year`/`month` in the body are ignored**; use `POST /` (form fields `station_id`/`year`/`month`/`unit`) to request a specific month. CLI (`get_tides.py`): `--unit` arg (choices: metric/imperial, default imperial). `scripts/update_example_image.sh` passes `--unit imperial` (Point Roberts is US). Playwright tests for the unit toggle must use `.check()` on the visible radio inputs (not hidden ones — these radios are visible, unlike the country-filter radios which are CSS-hidden).